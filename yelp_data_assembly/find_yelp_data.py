...
Also note that the function (rarely) can fail: This appears to happen when a
particular API call to the server fails to get a return, and can be corrected by
re-running the code for the desired city. In testing, this failures occured approx.
once every 7 cities, and never in the same location.
'''
...
# This may require creation of a free account
app_id = '######################'
app_secret = '################################################################'
yelp = Yelp(
    app_id,
